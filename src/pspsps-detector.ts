...
export type DetectorOptions = {
  ...
  export class PspspsDetector extends EventTarget {
    ...
    destroy() {
      ...
      if (this.debugEl && this.debugEl.parentElement) this.debugEl.parentElement.removeChild(this.debugEl);
      // @ts-ignore
      this.ctx = undefined;
    }
    ...
    private tick() {
      ...
      if (this.options.debug) this.updateDebug({ hi, lo, ratioDb, mean, std, flux, fMean, fStd, detected });
    }
    ...
    private mountDebug() {
      ...
      const fields = ['hi(dB)', 'lo(dB)', 'hi-lo(dB)', 'mean(dB)', 'std(dB)', 'flux', 'fMean', 'fStd', 'peaks'];
    ...
    private updateDebug(v: { hi:number; lo:number; ratioDb:number; mean:number; std:number; flux:number; fMean:number; fStd:number; detected:boolean; }) {
      ...
      const fluxThresh = v.fMean + (this.options.fluxKStd / this.options.sensitivity) * v.fStd;
...
export function usePspspsDetector(
    ...
    const start = useCallback(async () => {
      try {
        if (!detectorRef.current) {
          detectorRef.current = await PspspsDetector.create(stableOpts);
          const handler = () => onDetect && onDetect();
          detectorRef.current.addEventListener('pspsps-detected', handler as EventListener);
        }
    ...
    useEffect(() => {
      return () => { detectorRef.current?.destroy(); detectorRef.current = null; };
    }, []);
