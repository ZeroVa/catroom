...
export class PspspsDetector extends EventTarget {
  ...
  private prevFreq!: Float32Array; // for spectral flux
  ...
  private fluxEnv: number[] = []; // spectral flux envelope
  ...
  constructor(opts?: Partial<DetectorOptions>) {
    ...
    this.options = {
      ...
      hiBandHz: [3500, 10000],
      loBandHz: [80, 1500],
      envelopeWindow: 48, // slightly shorter window
      kStd: 0.9, // a bit looser
      ratioDbFloor: 3.5, // was 5
      hiDbFloor: -65, // was -58 (allow quieter rooms)
      minPeaks: 2, // was 3
      patternWindowMs: 1600, // was 1200
      minInterPeakMs: 60, // was 80
      maxInterPeakMs: 330, // was 250
      cooldownMs: 1400, // slight increase
      smoothingTimeConstant: 0.2, // was 0.6 (don’t smear peaks)
      debug: false,
      sensitivity: 1.0,

      // spectral flux settings
      fluxWindow: 48, // ~0.8s at rAF pace
      fluxKStd: 1.0,
      fluxFloor: 3.0, // minimal positive flux sum in dB units

      // mic DSP (off keeps bursts intact unless you’re playing audio)
  ...
  async init() {
    ...
    this.hiEnd = Math.min(
      ...
      Math.floor(hiHi / this.binHz)
    );
    this.loStart = Math.max(0, Math.floor(loLo / this.binHz));
    this.loEnd = Math.min(
      this.freqData.length - 1,
      Math.floor(loHi / this.binHz)
    );
  ...
  private spectralFluxHiBand(): number {
    // positive changes only, in dB units (OK for onset proxy)
    let flux = 0;
  ...
  private tick() {
    ...
    if (this.envelope.length > this.options.envelopeWindow)
      this.envelope.shift();
    const mean =
      this.envelope.reduce((a, b) => a + b, 0) / this.envelope.length;
    const std = Math.sqrt(
      this.envelope.reduce((a, b) => a + (b - mean) ** 2, 0) /
    ...
    const fStd = Math.sqrt(
      this.fluxEnv.reduce((a, b) => a + (b - fMean) ** 2, 0) /
        this.fluxEnv.length
    );
    ...
    // A peak is either: hi-band envelope pops OR spectral flux pops
    const hiPop = hi > Math.max(threshHi, this.options.hiDbFloor);
    const fluxPop = flux > Math.max(threshFlux, this.options.fluxFloor);

    if (
      (hiPop || fluxPop) &&
      ratioDb > this.options.ratioDbFloor / sens &&
      now - this.lastPeak > refractory
    ) {
      this.peaks.push(now);
      this.lastPeak = now;
    }

    // Keep peaks inside the pattern window
    while (
      this.peaks.length &&
      now - this.peaks[0] > this.options.patternWindowMs
    )
      this.peaks.shift();

    // Rhythm check (looser): need >= minPeaks within window with intervals in range
    let detected = false;
    if (this.peaks.length >= this.options.minPeaks) {
      const ok = this.peaks.slice(1).every((t, i) => {
        const dt = t - this.peaks[i];
        return (
          dt > this.options.minInterPeakMs && dt < this.options.maxInterPeakMs
        );
      });
      if (ok && now - this.lastFire > this.options.cooldownMs) {
        detected = true;
        this.lastFire = now;
  ...
  private mountDebug() {
    ...
    fields.forEach((f) => {
      const el = row(f);
      rows[f] = el.querySelector("span[data-v]") as HTMLSpanElement;
      d.appendChild(el);
    });
    ...
    spark.style.background = "rgba(255,255,255,0.06)";
    d.appendChild(spark);

    const hint = document.createElement("div");
    hint.style.opacity = "0.7";
    hint.style.marginTop = "6px";
    hint.innerHTML = `<b>tips</b>: short bursts “ps-ps-ps.” Flux spikes should jump during each burst.`;
    d.appendChild(hint);
  ...
  private updateDebug(v: {
    ...
    rows["fMean"].textContent = v.fMean.toFixed(2);
    rows["fStd"].textContent = v.fStd.toFixed(2);
    rows["peaks"].textContent = String((this as any).peaks?.length ?? 0);

    // sparkline: combine hi relative to thresh + flux relative to thresh
    const hiThresh =
      v.mean + (this.options.kStd / this.options.sensitivity) * v.std;
    const fluxThresh =
      v.fMean + (this.options.fluxKStd / this.options.sensitivity) * v.fStd;
    this.sparkData.push(v.hi - hiThresh + 0.5 * (v.flux - fluxThresh));
    ...
    ctx.globalAlpha = 1;
    ctx.beginPath();
    const mid = this.spark!.height / 2;
    const scale = 2.0;
    this.sparkData.forEach((val, i) => {
    ...
    if (v.detected) {
      this.debugEl!.style.boxShadow =
        "0 0 0 2px rgba(46,238,238,0.6), 0 8px 26px rgba(0,255,255,0.25)";
      setTimeout(() => {
        if (this.debugEl)
          this.debugEl.style.boxShadow = "0 6px 24px rgba(0,0,0,0.3)";
      }, 160);
    }
  }
}
